...
from metavision_core.event_io import DatWriter,H5EventsReader,H5EventsWriter

def write_event_hdf5(filename, ts, x, y, pol, width=None, height=None):
    """
    Writes event data to an HDF5 file using H5EventsWriter.

    Args:
        filename (str): Destination path for the HDF5 file.
        ts (numpy.ndarray): Timestamps as a one-dimensional numpy array.
        x (numpy.ndarray): X-coordinate values as a one-dimensional numpy array.
        y (numpy.ndarray): Y-coordinate values as a one-dimensional numpy array.
        pol (numpy.ndarray): Polarity values as a one-dimensional numpy array.
        width (int): Width of the recording (default is None).
        height (int): Height of the recording (default is None).
    """
    ...
    if height is None:
        height = np.max(y) + 1
    assert len(ts) == len(x) == len(y) == len(pol), "Input arrays must have the same length"

    # Create an instance of H5EventsWriter
    with H5EventsWriter(filename, height, width) as writer:
        # Prepare the events array
        events = np.zeros(len(ts), dtype=[('t', np.int64), ('x', np.float32), ('y', np.float32), ('pol', np.int32)])
        events['t'] = ts
        events['x'] = x
        events['y'] = y
        events['pol'] = pol

        # Write the events to the HDF5 file
        writer.write(events)
...
class EventsData:
    ...
    def display_events_metavision(self,evs,accumulation_time_us):
        ...
        with Window(title="Metavision SDK Get Started", width=self.width, height=self.height, mode=BaseWindow.RenderMode.BGR) as window:
            ...
            #     break
    def display_events(self,events,t_begin,t_end,downsample = False,width=1280, height=720):
    ...
    def display_events_3D(self,events,t_begin,t_end,width=1280, height=720):
        ...
        if events_filtered.size:
            ...
            # ON_index = np.where(events_filtered['p'] == 0)
            # colors[ON_index, :] = [30, 30, 220]  # Set red color for ON events

            # OFF_index = np.where(events_filtered['p'] == 1)
            # colors[OFF_index, :] = [200, 30, 30]  # Set green color for OFF events
...
def main():
    ...
    a = H5EventsReader("D:/2023/computional imaging/my_pbrt/output/Rotate_360_high/High_360_120deg.hdf5")
    print(a)
