...
#TODO
def MonteCarlo_read(impliciteFunc, pre_weight, N=10000):
    # Generate c(j) = probability(i < j) by cumulative sum and normalize
    cumulative_prob = np.cumsum(pre_weight)
    cumulative_prob /= cumulative_prob[-1]

    # Sample N points of i from 400 to 700 according to p(i)
    sampled_points = np.interp(np.random.rand(N), cumulative_prob, np.linspace(400, 700, len(pre_weight)))

    # Calculate sum(impliciteFunc[i] / p(i))
    weighted_sum = np.sum(impliciteFunc[sampled_points.astype(int)] / np.interp(sampled_points, np.linspace(400, 700, len(pre_weight)), pre_weight))

    # Calculate photocurrent
    photocurrent = weighted_sum / N
...
def calculate_intensity_from_spetral(exr_file, channel_number, BrightScale):
    ...
    if channel_number == 31:
        ...
        cv_image[:, :] =  (
            0.82 * channel_list[0] + 0.92 * channel_list[4] +
             0.98 * channel_list[8] + 
            1 * channel_list[12] + 
            0.99 * channel_list[16]  + 0.97 * channel_list[20] +
            0.87 * channel_list[24] +
            0.79 * channel_list[28] 
        )/8.0
        ...
        cv_image[:, :] =  (
            0.82 * channel_list[0] +  0.96 * channel_list[7] + 0.99 * channel_list[14]+0.94 * channel_list[21]+0.79 * channel_list[28]
        )/5.0
    elif channel_number == 4:
        cv_image[:, :] =  (
            0.82 * channel_list[0]  + 1 * channel_list[10]  +
            0.99 * channel_list[20]  + 0.85 * channel_list[30]
        )/4.0
    elif channel_number == 3:
        cv_image[:, :] =  (
            0.82 * channel_list[0] + 0.99 * channel_list[15]
            + 0.85 * channel_list[30]
        )/3.0
